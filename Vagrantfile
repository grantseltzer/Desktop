...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", privileged: false, inline: <<-SCRIPT
    sudo dnf update -y
    sudo dnf install -y make zsh go libbpf libbpf-devel clang podman util-linux-user glibc-devel.i686 strace glibc-static zlib-static llvm llvm-devel kernel-devel-$(uname -r)
